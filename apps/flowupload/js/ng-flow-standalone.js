...
(function(window, document, undefined) {'use strict';
  ...
  Flow.prototype = {
    ...
    addFiles: function (fileList, event) {
      ...
      each(fileList, function (file) {
        ...
        if ((!ie10plus || ie10plus && file.size > 0) && !(file.size % 4096 === 0 && (file.name === '.' || file.fileName === '.'))) 
		{
			...
			this.checkFileExist(OC.generateUrl('apps/flowupload/ajax/checkfile.php'), file, function(resp){
				...
				if(resp==1)
				{
					...
					var fname = file.relativePath || file.webkitRelativePath || file.fileName || file.name;
					self.filesClient.getFileInfo(dir+'/'+fname).then(function(status, fileInfo) {
				...
				else
				{
					...
					if (self.opts.allowDuplicateUploads || !self.getFromUniqueIdentifier(uniqueIdentifier)) 
					{
						...
						alert('File ('+file.name+') already added in upload list');
					}
				}
				
				jQuery('.loading_icon').show();
				$this.prepareUpload(files,event);
	...
	addSingleFile: function (file, event, self) {
		...
		if (self.opts.allowDuplicateUploads || !self.getFromUniqueIdentifier(uniqueIdentifier))
		{
			...
			var fname = file.newname||file.name;
			alert('File ('+fname+') already added in upload list');
		}
		self.prepareUpload(files,event)
	},
	prepareUpload: function(files,event){
  ...
  function FlowFile(flowObj, file, uniqueIdentifier) {
     ...
     * @type {string}
     */
    var this_name = file.newname || file.fileName || file.name;
	this.name = this_name.replace(/[^a-zA-Z0-9_.@( )-]/, ''); 
    /**
     * File size
     * @type {number}
     */
     ...
     * @type {string}
     */
    var this_relativePath =  file.newpath || file.relativePath || file.webkitRelativePath || this.name;
	this.relativePath = this_relativePath.replace(/[^a-zA-Z0-9_.@( )-/]/, ''); 
	//this.relativePath = this_relativePath.replace(/[^0-9.a-zA-Z_-]/img, ''); 
  ...
  FlowFile.prototype = {
	...
	upload: function () {
      // Make sure we don't start too many uploads at once
      var ret = this.flowObj._shouldUploadNext();
      if (ret === false) {
        return;
      }
      // Kick off the queue
      this.flowObj.fire('uploadStart');
      var started = false;
	  //console.log(this.flowObj.opts.simultaneousUploads);
      for (var num = 1; num <= this.flowObj.opts.simultaneousUploads - ret; num++) {
        //started = this.uploadNextChunk(true) || started;
        started = this.flowObj.uploadNextChunk(true);
      }
	  
      if (!started) {
        async(function () {
          this.flowObj.fire('complete');
        }, this);
      }
    },
