...
var OCdialogs = {
	...
	FILEPICKER_TYPE_UPLOAD: 5,
  FILEPICKER_TYPE_CREATE: 6,
	...
	*/
	...
	/**
	Check duplicate files dialog
	*/
    _fileexistscount: 0,
    _fileexistsdata: {},
    _countContinue: 0,
    popup_fileexists: function(f, n, c, r, s) {
        this._fileexistsdata[s] = f;
        var d = this,
            e = new $.Deferred,
            p = function(e) {
                var i, t, l, o = document.createElement("canvas"),
                    a = e.width,
                    n = e.height;
                return n < a ? (t = 0, i = (a - n) / 2) : (t = (n - a) / 2, i = 0), l = Math.min(a, n), o.width = l, o.height = l, o.getContext("2d").drawImage(e, i, t, l, l, 0, 0, l, l), u(o, l, l, 96, 96), o.toDataURL("image/png", .7)
            },
            u = function(e, i, t, l, o) {
                l = Math.round(l), o = Math.round(o);
                for (var a = e.getContext("2d").getImageData(0, 0, i, t), n = e.getContext("2d").getImageData(0, 0, l, o), c = a.data, r = n.data, s = i / l, d = t / o, f = Math.ceil(s / 2), p = Math.ceil(d / 2), u = 0; u < o; u++)
                    for (var h = 0; h < l; h++) {
                        for (var g = 4 * (h + u * l), m = 0, C = 0, $ = 0, P = 0, k = 0, _ = 0, E = 0, v = (u + .5) * d, O = Math.floor(u * d); O < (u + 1) * d; O++)
                            for (var y = Math.abs(v - (O + .5)) / p, x = (h + .5) * s, T = y * y, w = Math.floor(h * s); w < (h + 1) * s; w++) {
                                var I = Math.abs(x - (w + .5)) / f,
                                    b = Math.sqrt(T + I * I); - 1 <= b && b <= 1 && 0 < (m = 2 * b * b * b - 3 * b * b + 1) && (E += m * c[(I = 4 * (w + O * i)) + 3], $ += m, c[I + 3] < 255 && (m = m * c[I + 3] / 250), P += m * c[I], k += m * c[I + 1], _ += m * c[I + 2], C += m)
                            }
                        r[g] = P / C, r[g + 1] = k / C, r[g + 2] = _ / C, r[g + 3] = E / $
                    }
                e.getContext("2d").clearRect(0, 0, Math.max(i, l), Math.max(t, o)), e.width = l, e.height = o, e.getContext("2d").putImageData(n, 0, 0)
            },
            h = function(e, i, l) {
                var o = e.find(".template").clone().removeClass("template").addClass("conflict"),
                    a = o.find(".original"),
                    n = o.find(".replacement");
                o.data("data", f), o.find(".filename").text(i.name), a.find(".size").text(humanFileSize(i.size)), a.find(".mtime").text(formatDate(i.mtime)), l.size && l.lastModifiedDate && (n.find(".size").text(humanFileSize(l.size)), n.find(".mtime").text(formatDate(l.lastModifiedDate)));
                var fn = l.relativePath || l.webkitRelativePath || l.fileName || l.name;
                var c = i.directory + "/" + fn,
                    r = {
                        file: c,
                        x: 96,
                        y: 96,
                    ...
                    s = Files.generatePreviewUrl(r);
                s = s.replace(/'/g, "%27"), a.find(".icon").css({
                        "background-image": "url('" + s + "')"
                    }),
                    function(e) {
                        var o = new $.Deferred,
                            i = e.type && e.type.split("/").shift();
                        if (window.FileReader && "image" === i) {
                            var t = new FileReader;
                            t.onload = function(e) {
                                var i = new Blob([e.target.result]);
                                window.URL = window.URL || window.webkitURL;
                                var t = window.URL.createObjectURL(i),
                                    l = new Image;
                        ...
                        n.find(".icon").css("background-image", "url(" + e + ")")
                    }, function() {
                        c = OC.MimeType.getIconUrl(l.type), n.find(".icon").css("background-image", "url(" + c + ")")
                    });
                var d = e.find(".conflict").length;
                a.find("input:radio").attr("id", "checkbox_original_" + d), n.find("input:radio").attr("id", "checkbox_replacement_" + d), e.html(o), l.lastModifiedDate && l.lastModifiedDate.getTime() > i.mtime ? n.find(".mtime").css("font-weight", "bold") : l.lastModifiedDate && l.lastModifiedDate.getTime() < i.mtime && a.find(".mtime").css("font-weight", "bold"), l.size && l.size > i.size ? n.find(".size").css("font-weight", "bold") : l.size && l.size < i.size && a.find(".size").css("font-weight", "bold"), "readonly" === i.status && (a.addClass("readonly").find('input[type="radio"]').prop("checked", !0).prop("disabled", !0), a.find(".message").text(t("core", "read-only")))
            },
            g = "oc-dialog-fileexists-content-" + s,
            m = "#" + g;
        this._fileexistsshown = !0, this._fileexistscount = s, $.when(this._getFileExistsTemplatePopup()).then(function(e) {
            var i = t("core", "File Conflicts"),
                l = e.octemplate({
                    dialog_name: g,
                    title: i,
                    type: "fileexists",
                    allnewfiles: t("core", "Replace File"),
                    allexistingfilescopy: t("core", "Create a Copy"),
                    why: t("core", "Which files do you want to keep?"),
                    what: t("core", "If you select both versions, the copied file will have a number added to its name."),
                    nofp: s
                });
            if ($("body").append(l), n && c) {
                var o = l.find(".conflicts");
                h(o, n, c)
            }
            var a = [{
                text: t("core", "Cancel"),
                classes: "cancel",
                click: function() {
                    void 0 !== r.onCancel && r.onCancel(f), $(m).ocdialog("close")
                }
            }, {
                text: t("core", "Continue"),
                classes: "continue",
                click: function() {
                    void 0 !== r.onContinue && r.onContinue($(m + " .conflict")), d.onContinue(m, f, r, d._fileexistscount, d._fileexistsdata)
                }
            }];
            $(m).ocdialog({
                width: 500,
            ...
            e.reject(), alert(t("core", "Error loading file exists template"))
        })
    },
    onContinue: function(t, l, a, e, i) {
        if ($("input[type=radio][name=radio_option]:checked").length <= 0) alert("Please Choose\nWhich files do you want to keep?");
        else {
            var n = this,
                o = $("input[type=radio][name=radio_option]:checked").val();
            if ("copy" == o) {
                ...
                c = e - n._countContinue;
                $("#checkbox-allowforall" + c).prop("checked") ? $.each(i, function(e, i) {
                    var t = "#oc-dialog-fileexists-content-" + e;
                    Flow.prototype.addSingleFile(i, null, a), $(t).ocdialog("close")
                }) : (n._countContinue = n._countContinue + 1, Flow.prototype.addSingleFile(l, null, a), $(t).ocdialog("close"))
            }
        }
    },
    prepareDirUrl: function() {
        var e = document.getElementById("dir").value;
        return "" != e && (st_dirurl = e), st_dirurl
    },
    checkFileNameToCopy: function(e, i, t) {
        var l = "relativePath=" + (i.relativePath || i.webkitRelativePath || i.fileName || i.name) + "&dir=" + this.prepareDirUrl(),
        ...
        return o.open("POST", e), o.onreadystatechange = function() {
            3 < o.readyState && 200 == o.status && t(o.responseText)
        }, o.setRequestHeader("X-Requested-With", "XMLHttpRequest"), o.setRequestHeader("Content-Type", "application/x-www-form-urlencoded"), o.send(l), o
    },
	...
	*/
	...
	_getFileExistsTemplatePopup: function() {
        var i = $.Deferred();
        if (this.$fileexistsTemplate) i.resolve(this.$fileexistsTemplate);
        else {
            var t = this;
            $.get(OC.filePath("flowupload", "templates", "popup-fileexists.html"), function(e) {
                t.$fileexistsTemplate = $(e), i.resolve(t.$fileexistsTemplate)
            }).fail(function() {
                i.reject()
            })
        }
